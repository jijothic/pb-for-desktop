...
let parseArchitecture = function(search) {
    return search.match(new RegExp(nodeArchs.join('|')));
};
...
let packageArtifact = function(platformName, sourceArtifact, targetDirectory, buildOptions, callback) {
    ...
    platformPackager.darwin = function() {
        let architectureName = parseArchitecture(sourceArtifact),
            targetFileName = getSafePackageFileName(buildOptions['name'], platformName, buildOptions['app-version'], architectureName, null, true),
    ...
    platformPackager.win32 = function() {
        let architectureName = parseArchitecture(sourceArtifact),
            targetFileName = getSafePackageFileName(buildOptions['productName'], platformName, buildOptions['app-version'], architectureName),
            targetSubdirectory = path.join(targetDirectory, targetFileName),
            targetExtension = '.exe',
    ...
    platformPackager.linux = function() {
        ...
        let installerOptions = {
            ...
            rename: function(dest) {
                ...
                let filename = getSafePackageFileName(buildOptions['name'] + targetExtension, platformName, buildOptions['app-version'], architectureName);
