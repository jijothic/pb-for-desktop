...
let createGithubOptions = function(assetList, githubToken) {
    if (!assetList || !_.isArray(assetList)) {
        logger.logErr('error', 'assetList missing or wrong format.');
        return;
    }

    if (_.isEmpty(assetList)) {
        logger.logErr('error', 'assetList is empty.');
        return;
    }

    if (!githubToken || !_.isString(githubToken)) {
        logger.logErr('error', 'githubToken missing or wrong format.');
        return;
    }
...
let addEventHandlers = function(release) {
    if (!release || !_.isObject(release)) {
        logger.logErr('error', 'release missing or wrong format.');
        return;
    }
    ...
    release.on('reuse-release', function() {
        logger.log('Github Release', 'exists');
    });

    release.on('upload-asset', function(name) {
        logger.log('Github Upload', 'starting', name);
    });
    ...
    release.on('uploaded-asset', function(name) {
        ...
        logger.log('Github Upload', 'complete', name);
    });
...
if (require.main === module) {
    releaseOnGithub(function(err, result) {
        if (err) {
            logger.logErr('Github Release', 'failed', err);
            return process.exit(1);
        }
        logger.log('Github Release', 'successful', result['url']);
